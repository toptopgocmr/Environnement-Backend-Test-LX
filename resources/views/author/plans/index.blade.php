...
@section('title', 'Mes Forfaits – LireX')
@section('page-title', 'Forfaits de publication')
@section('page-subtitle', 'Choisissez le forfait adapté à vos ambitions')
...
<div class="mb-8 p-5 rounded-2xl border flex items-center gap-5"
  ...
  <div class="flex-1">
    ...
    <p class="text-white font-bold text-xl">{{ $activePlan->plan->name }}</p>
    <p class="text-slate-400 text-sm mt-0.5">
      Facturation {{ $activePlan->billing === 'annual' ? 'annuelle' : 'mensuelle' }}
      · Expire le {{ $activePlan->ends_at?->format('d/m/Y') ?? '–' }}
      · {{ number_format($activePlan->plan->royalty_percent, 0) }}% de royalties
    </p>
  </div>
  <span class="px-3 py-1 rounded-full text-xs font-bold
    {{ $activePlan->status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-amber-500/20 text-amber-400' }}">
    {{ $activePlan->status === 'active' ? '✓ Actif' : '⏳ En attente de validation' }}
  </span>
</div>
@else
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-{{ min($plans->count(), 4) }} gap-5">
  ...
  <div class="relative rounded-2xl border flex flex-col transition-all hover:shadow-lg
    {{ $isFeatured ? 'border-blue-500 shadow-blue-100 shadow-lg ring-2 ring-blue-500/30' : 'border-slate-200' }}"
    ...
    <div class="p-6 border-b border-slate-100">
      ...
      <div class="mt-4">
        <div id="price-monthly-{{ $plan->id }}">
          @if($plan->price_monthly == 0)
            <span class="text-3xl font-black text-slate-800">Gratuit</span>
          @else
            <span class="text-3xl font-black text-slate-800">{{ number_format($plan->price_monthly, 0, ',', ' ') }}</span>
            <span class="text-slate-500 text-sm"> {{ $plan->currency }}/mois</span>
          @endif
        </div>
        <div id="price-annual-{{ $plan->id }}" class="hidden">
          @if($plan->price_annual == 0)
            <span class="text-3xl font-black text-slate-800">Gratuit</span>
          @else
            <span class="text-3xl font-black text-blue-600">{{ number_format($plan->price_annual, 0, ',', ' ') }}</span>
            <span class="text-slate-500 text-sm"> {{ $plan->currency }}/an</span>
          @endif
        </div>
      </div>
    </div>
    ...
    <div class="p-6 flex-1 space-y-3">
      <div class="flex items-center gap-2 text-sm text-slate-600">
        ...
        <span>{{ $plan->max_books === -1 ? 'Livres illimités' : $plan->max_books . ' livre(s) max' }}</span>
      </div>
      <div class="flex items-center gap-2 text-sm text-slate-600">
        <i class="fa-solid fa-file-arrow-up text-blue-500 w-4"></i>
        <span>Fichiers jusqu'à {{ $plan->max_file_size_mb }} Mo</span>
      </div>
      <div class="flex items-center gap-2 text-sm {{ $plan->royalty_percent >= 80 ? 'text-green-700 font-semibold' : 'text-slate-600' }}">
        <i class="fa-solid fa-percent text-green-500 w-4"></i>
        <span>{{ number_format($plan->royalty_percent, 0) }}% de royalties</span>
      </div>
      <div class="flex items-center gap-2 text-sm {{ $plan->allow_physical ? 'text-slate-700' : 'text-slate-400' }}">
        <i class="fa-solid fa-truck w-4 {{ $plan->allow_physical ? 'text-blue-500' : 'text-slate-300' }}"></i>
        <span class="{{ !$plan->allow_physical ? 'line-through' : '' }}">Vente physique</span>
      </div>
      <div class="flex items-center gap-2 text-sm {{ $plan->allow_academic ? 'text-slate-700' : 'text-slate-400' }}">
        <i class="fa-solid fa-graduation-cap w-4 {{ $plan->allow_academic ? 'text-blue-500' : 'text-slate-300' }}"></i>
        <span class="{{ !$plan->allow_academic ? 'line-through' : '' }}">Thèses & mémoires</span>
      </div>
      <div class="flex items-center gap-2 text-sm {{ $plan->ai_review ? 'text-slate-700' : 'text-slate-400' }}">
        <i class="fa-solid fa-robot w-4 {{ $plan->ai_review ? 'text-purple-500' : 'text-slate-300' }}"></i>
    ...
    <div class="p-6 pt-0">
      ...
      @else
      <button type="button"
        onclick="openPayModal({{ $plan->id }},'{{ addslashes($plan->name) }}',{{ $plan->price_monthly }},{{ $plan->price_annual }},'{{ $plan->currency }}')"
        class="w-full py-2.5 rounded-xl text-sm font-semibold transition-all
          {{ $isFeatured ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-md' : 'bg-slate-800 text-white hover:bg-slate-900' }}">
        {{ $plan->price_monthly == 0 ? 'Choisir ce forfait' : 'Souscrire' }} →
      </button>
      @endif
    </div>
  </div>
...
<div id="payModal" class="fixed inset-0 z-50 hidden items-center justify-center"
  ...
  <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden flex flex-col" style="max-height:90vh;">
    ...
    <form id="pay-form" method="POST" action="{{ route('author.plans.subscribe') }}" class="overflow-y-auto flex-1">
      @csrf
      ...
      {{-- ÉTAPE 2 : Saisie numéro + déclenchement push payment --}}
      <div id="step-2" class="p-4 hidden">
        ...
        <div id="pay-momo">
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 class="font-bold text-slate-800" id="momo-title">Mobile Money (Peex)</h3>
          ...
          <div class="mb-4">
            ...
            <p class="text-xs text-slate-400 mt-1.5">
              <span id="momo-phone-hint">Le numéro Mobile Money enregistré sur votre téléphone.</span>
            </p>
          </div>
      ...
      {{-- ÉTAPE 3 : Statut du paiement push (polling temps réel) --}}
...
async function triggerPushPayment() {
  ...
  try {
    const res = await fetch('{{ route("author.plans.pay") }}', {
      method: 'POST',
      headers: {
