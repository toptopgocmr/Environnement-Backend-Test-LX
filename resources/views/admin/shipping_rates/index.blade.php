...
@section('title', 'Frais de livraison – LireX Admin')
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="px-6 py-4 flex items-center justify-between
                  {{ $rate->zone === 'congo' ? 'bg-green-50 border-b border-green-100' : 'bg-blue-50 border-b border-blue-100' }}">
        <div class="flex items-center gap-3">
          <span class="text-2xl">{{ $rate->zone === 'congo' ? '🇨🇬' : '🌍' }}</span>
          <div>
            <p class="font-bold text-gray-800">{{ $rate->label }}</p>
            <p class="text-xs text-gray-500">Zone : {{ $rate->zone }}</p>
          </div>
        </div>
        <span class="px-3 py-1 rounded-full text-xs font-semibold
                     {{ $rate->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600' }}">
      ...
      <div class="px-6 py-5 grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <p class="font-bold text-gray-800 text-lg">{{ number_format($rate->base_price, 0, ',', ' ') }} XAF</p>
        </div>
        <div>
          <p class="text-gray-400 text-xs uppercase tracking-wide">+/kg suppl.</p>
          <p class="font-bold text-gray-800 text-lg">{{ number_format($rate->price_per_kg, 0, ',', ' ') }} XAF</p>
        </div>
        <div>
          <p class="text-gray-400 text-xs uppercase tracking-wide">Gratuit dès</p>
          <p class="font-semibold {{ $rate->free_above > 0 ? 'text-green-600' : 'text-gray-400' }}">
            {{ $rate->free_above > 0 ? number_format($rate->free_above, 0, ',', ' ') . ' XAF' : 'Jamais' }}
          </p>
        </div>
        <div>
          <p class="text-gray-400 text-xs uppercase tracking-wide">Délai estimé</p>
          <p class="font-semibold text-gray-800">{{ $rate->delivery_range }}</p>
        </div>
        @if($rate->notes)
        <div class="col-span-2">
          <p class="text-gray-400 text-xs uppercase tracking-wide">Notes</p>
          <p class="text-gray-600 text-sm">{{ $rate->notes }}</p>
        </div>
        @endif
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <select id="sim-zone" class="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
          @foreach($rates->where('is_active', true) as $rate)
            <option value="{{ $rate->id }}" data-base="{{ $rate->base_price }}" data-pkg="{{ $rate->price_per_kg }}" data-free="{{ $rate->free_above }}">
              {{ $rate->label }}
            </option>
          @endforeach
        </select>
...
<div id="modal-add" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
  <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
    ...
    <form method="POST" action="{{ route('admin.shipping-rates.store') }}">
      @csrf
      @include('admin.shipping_rates._form')
...
<div id="modal-edit" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
  <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
    ...
    <form id="edit-form" method="POST">
      @csrf
      @include('admin.shipping_rates._form', ['edit' => true])
      <div class="flex gap-3 mt-6">
