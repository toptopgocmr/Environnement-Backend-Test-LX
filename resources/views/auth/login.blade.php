...
<body class="flex items-center justify-center min-h-screen px-4 py-10">
    ...
    <div class="card">
        ...
        <div class="grid grid-cols-3 gap-1.5 p-1 mb-6 rounded-xl" style="background:#f1f5f9;">
            ...
            <button type="button" onclick="window.location.href='{{ rtrim(config('app.frontend_url'), '/') }}/login'"
                class="py-2 rounded-lg text-xs font-semibold transition" style="color:#64748b;">
                Espace Lecture
            </button>
        </div>
        ...
        @if(session('status'))
            <div class="bg-green-50 border border-green-200 text-green-700 rounded-xl p-3 mb-4 text-sm">{{ session('status') }}</div>
        @endif
        @if($errors->any())
        ...
        <form action="{{ route('login.post') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required
                    placeholder="admin@lirex.africa" class="field">
            </div>
            <div>
            ...
            <div class="flex items-center justify-between text-sm">
                <label class="flex items-center gap-2 text-slate-500 cursor-pointer select-none">
                    <input type="checkbox" name="remember" class="rounded accent-amber-500"> Se souvenir de moi
                </label>
                <a href="{{ route('password.request') }}" class="text-amber-600 hover:text-amber-700 font-medium">Mot de passe oublié ?</a>
            </div>
        ...
        <div class="mt-5 pt-5 border-t border-slate-100 text-center text-sm text-slate-500" id="register-hint">
            ...
            <a href="{{ route('register') }}" class="text-amber-600 font-semibold hover:text-amber-700 ml-1">S'inscrire</a>
        </div>
