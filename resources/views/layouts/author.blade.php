...
<head>
  ...
  <title>@yield('title', 'LireX – Espace Auteur')</title>
...
<header class="aws-topbar">
  ...
  {{-- Logo --}}
  <a href="{{ route('author.dashboard') }}" class="aws-topbar-logo">
  ...
  <div class="aws-divider"></div>
  <a href="{{ route('author.books.index') }}"   class="aws-nav-link">Mes livres</a>
  <a href="{{ route('author.customers.index') }}" class="aws-nav-link">Clients</a>
  <a href="{{ route('author.earnings.index') }}" class="aws-nav-link">Revenus</a>
  <a href="{{ route('author.plans.index') }}"    class="aws-nav-link">Forfaits</a>

  {{-- Right --}}
  <div class="aws-topbar-right">
    {{-- Messages bell --}}
    @php $unreadCount = \App\Models\ChatConversation::whereHas('participants', fn($q) => $q->where('user_id', auth()->id()))->withCount(['messages as unread' => fn($q) => $q->where('sender_id','!=',auth()->id())->where('is_read',false)])->get()->sum('unread'); @endphp
    <a href="{{ route('author.chat.index') }}" class="aws-icon-btn" style="position:relative;">
      <i class="far fa-comment-dots"></i>
      @if($unreadCount > 0)
        <span style="position:absolute;top:2px;right:2px;background:#ef4444;color:#fff;font-size:.55rem;font-weight:800;border-radius:50%;width:14px;height:14px;display:flex;align-items:center;justify-content:center;">{{ $unreadCount > 9 ? '9+' : $unreadCount }}</span>
      @endif
    </a>
      ...
      <div id="lang-dropdown-author" style="display:none;position:absolute;right:0;top:calc(100% + 6px);background:#fff;border:1px solid #d5d9d9;border-radius:6px;box-shadow:0 4px 16px rgba(0,0,0,.18);min-width:190px;z-index:200;max-height:340px;overflow-y:auto;">
        ...
        <a href="{{ route('locale.set', $code) }}"
           style="display:flex;align-items:center;gap:9px;padding:7px 12px;font-size:.8rem;text-decoration:none;transition:background .1s;
                  {{ $current === $code ? 'background:#e8f4fd;font-weight:600;' : 'color:#16191f;' }}">
          <span style="font-size:1rem;">{{ $meta['flag'] }}</span>
          <span style="flex:1;{{ $current === $code ? 'color:#0073bb;' : '' }}">{{ $meta['name'] }}</span>
          <span style="font-size:.65rem;font-weight:700;padding:1px 5px;border-radius:3px;
                       {{ $current === $code ? 'background:#0073bb;color:#fff;' : 'background:#e8eaed;color:#545b64;' }}">
            {{ $meta['code'] }}
          </span>
        </a>
        @endforeach
    ...
    <div style="position:relative;">
      <button onclick="toggleDropdown()" class="aws-icon-btn" style="gap:7px;">
        ...
        <span style="color:#fff;font-size:.8rem;max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ auth()->user()->name }}</span>
      ...
      <div id="user-dropdown">
        <div style="padding:10px 14px;border-bottom:1px solid #f0f0f0;">
          <p style="font-size:.82rem;font-weight:700;color:#16191f;">{{ auth()->user()->name }}</p>
          <p style="font-size:.72rem;color:#545b64;">{{ auth()->user()->email }}</p>
        </div>
        <a href="{{ route('author.profile.edit') }}" class="dd-item"><i class="far fa-user"></i> Mon profil</a>
        <a href="{{ route('author.chat.index') }}"   class="dd-item"><i class="far fa-comment-dots"></i> Messages</a>
        <a href="{{ route('author.plans.index') }}"  class="dd-item"><i class="fas fa-crown" style="color:#ff9900;"></i> Mon forfait</a>
        <hr class="dd-sep">
...
<aside class="aws-sidebar">
  ...
  @php
    $chatUnread = \App\Models\ChatConversation::whereHas('participants', fn($q) => $q->where('user_id', auth()->id()))
      ...
      ->get()->sum('unread');
  @endphp
  <a href="{{ route('author.chat.index') }}"
     class="nav-item {{ request()->routeIs('author.chat.*') ? 'active' : '' }}">
    <i class="far fa-comment-dots"></i> Messages
    @if($chatUnread > 0)
      <span class="nav-badge-red">{{ $chatUnread > 9 ? '9+' : $chatUnread }}</span>
    @endif
  </a>

  <div class="nav-section">Finance</div>
  <a href="{{ route('author.earnings.index') }}"
     class="nav-item {{ request()->routeIs('author.earnings.*') ? 'active' : '' }}">
    <i class="fas fa-chart-line"></i> Revenus &amp; Retraits
  </a>

  <div class="nav-section">Abonnement</div>
  <a href="{{ route('author.plans.index') }}"
     class="nav-item {{ request()->routeIs('author.plans.*') ? 'active' : '' }}">
    <i class="fas fa-crown" style="color:#ff9900;"></i> Mes Forfaits
  </a>

  <div class="nav-section">Compte</div>
  <a href="{{ route('author.profile.edit') }}"
     class="nav-item {{ request()->routeIs('author.profile.*') ? 'active' : '' }}">
  ...
  <div style="margin-top:auto; padding:16px; border-top:1px solid rgba(255,255,255,.06); margin-top:20px;">
    <p style="font-size:.65rem; color:rgba(255,255,255,.25); line-height:1.4;">
      &copy; {{ date('Y') }} LireX<br>
      @if(auth()->user()->currentPlan)
        <span style="color:#ff9900;">{{ auth()->user()->currentPlan->plan->name ?? 'Plan actif' }}</span>
      @endif
    </p>
  </div>
</aside>
...
<main class="aws-main">
  ...
  <div class="aws-page-header">
    <div class="aws-breadcrumb">
      <a href="{{ route('author.dashboard') }}">LireX</a> › Espace Auteur
      @hasSection('breadcrumb') › @yield('breadcrumb') @endif
    </div>
    <div style="display:flex; align-items:center; justify-content:space-between;">
      <div>
        <h1 class="aws-page-title">@yield('page-title', 'Tableau de bord')</h1>
        @hasSection('page-subtitle')<p class="aws-page-sub">@yield('page-subtitle')</p>@endif
      </div>
  ...
  <div style="padding:0 24px; margin-top:12px;">
    @if(session('success'))
      <div class="aws-alert-ok"><i class="fas fa-check-circle"></i> {{ session('success') }}</div>
    @endif
    @if(session('error'))
      <div class="aws-alert-err"><i class="fas fa-exclamation-circle"></i> {{ session('error') }}</div>
    @endif
    @if($errors->any())
      <div class="aws-alert-err"><i class="fas fa-exclamation-circle"></i> {{ $errors->first() }}</div>
    @endif
  </div>
  ...
  <footer style="padding:12px 24px; border-top:1px solid #d5d9d9; background:#fff; font-size:.72rem; color:#545b64; display:flex; justify-content:space-between; align-items:center;">
    <span>&copy; {{ date('Y') }} LireX &mdash; Espace Auteur</span>
