...
<head>
  ...
  <title>@yield('title', 'LireX Admin')</title>
...
<body class="h-full">
  ...
  <nav class="aws-topbar">
    ...
    {{-- Logo --}}
    <a href="{{ route('admin.dashboard') }}" class="aws-topbar-logo">
    ...
    <a href="{{ route('admin.books.index') }}"       class="aws-nav-link">Livres</a>
    <a href="{{ route('admin.users.index') }}"       class="aws-nav-link">Utilisateurs</a>
    <a href="{{ route('admin.orders.index') }}"      class="aws-nav-link">Commandes</a>
    <a href="{{ route('admin.withdrawals.index') }}" class="aws-nav-link">Finance</a>
    <a href="{{ route('admin.payments.index') }}"    class="aws-nav-link" style="position:relative;">
      Paiements
      @php $pendingCount = \App\Models\AuthorPlan::where('status','pending_payment')->count(); @endphp
      @if($pendingCount > 0)
      <span style="position:absolute;top:-4px;right:-8px;background:#ef4444;color:#fff;font-size:.6rem;font-weight:700;padding:1px 5px;border-radius:10px;line-height:1.4;">{{ $pendingCount }}</span>
      @endif
    </a>
    <a href="{{ route('admin.plans.index') }}"       class="aws-nav-link">Forfaits</a>
    ...
    <div class="aws-topbar-right">
      ...
      <div class="aws-divider"></div>
      <a href="{{ route('admin.settings.index') }}" class="aws-icon-btn" title="Paramètres">
  ...
  <aside class="aws-sidebar">
    ...
    <a href="{{ route('admin.books.index') }}" class="nav-item {{ request()->routeIs('admin.books.*') ? 'active' : '' }}">
      ...
      @php $pending = \App\Models\Book::pending()->count(); @endphp
      @if($pending > 0)<span class="nav-badge">{{ $pending }}</span>@endif
    </a>
    <a href="{{ route('admin.ai-reviews.index') }}" class="nav-item {{ request()->routeIs('admin.ai-reviews.*') ? 'active' : '' }}">
    ...
    <a href="{{ route('admin.accounts.index') }}" class="nav-item {{ request()->routeIs('admin.accounts.*') ? 'active' : '' }}">
      ...
      @php $pendingAcc = \App\Models\AccountRequest::where('status','pending')->count(); @endphp
      @if($pendingAcc > 0)<span class="nav-badge">{{ $pendingAcc }}</span>@endif
    </a>
    ...
    <a href="{{ route('admin.physical.orders') }}" class="nav-item {{ request()->routeIs('admin.physical.orders') ? 'active' : '' }}">
      <i class="fa-solid fa-truck"></i> Commandes physiques
    </a>
    <a href="{{ route('admin.physical.stock') }}" class="nav-item {{ request()->routeIs('admin.physical.stock') ? 'active' : '' }}">
      <i class="fa-solid fa-boxes-stacked"></i> Stock physique
    </a>
    <a href="{{ route('admin.shipping-rates.index') }}" class="nav-item {{ request()->routeIs('admin.shipping-rates.*') ? 'active' : '' }}">
    ...
    <a href="{{ route('admin.withdrawals.index') }}" class="nav-item {{ request()->routeIs('admin.withdrawals.*') ? 'active' : '' }}">
      ...
      @if($pendingW > 0)<span class="nav-badge nav-badge-red">{{ $pendingW }}</span>@endif
    </a>
    ...
    <div style="margin-top:24px; padding:12px 16px; border-top:1px solid rgba(255,255,255,.08);">
      <div style="display:flex; align-items:center; gap:9px;">
        <img src="{{ Auth::user()->avatar_url }}" style="width:32px; height:32px; border-radius:50%; object-fit:cover; border:2px solid var(--aws-orange);" alt="">
        <div style="min-width:0;">
          <p style="color:#fff; font-size:.78rem; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ Auth::user()->name }}</p>
  ...
  <div class="aws-main">
    ...
    {{-- Page header (AWS breadcrumb style) --}}
    <div class="aws-page-header">
      <div class="aws-breadcrumb">
        <a href="{{ route('admin.dashboard') }}">LireX</a>
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          @hasSection('page-subtitle')
            <p class="aws-page-sub">@yield('page-subtitle')</p>
          @endif
        </div>
    ...
    <footer style="padding:14px 24px; border-top:1px solid #d5d9d9; background:#fff; font-size:.72rem; color:#6b7280; display:flex; align-items:center; justify-content:space-between;">
      <span>© {{ date('Y') }} LireX — Administration &nbsp;·&nbsp; Développé avec ❤ par <strong style="color:#374151;">Basile Marius NGASSAKI ZONI</strong></span>
    </footer>
  </div>
