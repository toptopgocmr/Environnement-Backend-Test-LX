...
<div>
    ...
    <textarea name="description" rows="3"
        class="w-full px-4 py-3 rounded-xl text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        style="background:#162035;border:1px solid #1E3A6A" placeholder="Description de la catégorie..."><?php echo e(old('description', $category->description ?? '')); ?></textarea>
</div>
<div class="grid grid-cols-2 gap-4">
    ...
    <div>
        ...
        <input type="color" name="color" value="<?php echo e(old('color', $category->color ?? '#2563EB')); ?>"
            class="w-full h-12 rounded-xl cursor-pointer" style="background:#162035;border:1px solid #1E3A6A;padding:4px">
    </div>
</div>
<div class="flex items-center gap-3">
    <input type="checkbox" name="is_active" id="is_active" value="1" <?php echo e(old('is_active', $category->is_active ?? true) ? 'checked' : ''); ?>

        class="w-4 h-4 rounded">
    <label for="is_active" class="text-slate-300 text-sm">Catégorie active</label>
</div>
<?php /**PATH C:\Users\Lenovo Yoga\Downloads\LireX-APP\backend\resources\views/admin/categories/_form.blade.php ENDPATH**/ ?>